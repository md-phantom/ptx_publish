...
class PtxGenerateMeshCache(Passive):
    """
    * Handle making the model passive in Maya
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._cache_nodes = kwargs.get('nodes_to_cache') if 'nodes_to_cache' in kwargs.keys() else [node for node in pm.ls(assemblies=True) if node not in self.__ignore_assemblies__]
    ...
    def make_passive(self):        
        ...
        self.__exporter = prc_factory.create(exp_mod, root_node=self._cache_nodes, export_path=self.export_path, frame_range=self.frame_range)
...
class PtxNodeBuilder:
    ...
    * Factory for Creating an instance of the Maya Model Activate class.
    ...
    def __call__(self, *args, **kwds) -> PtxGenerateMeshCache:
        if not self._instance:
            self._instance = PtxGenerateMeshCache(*args, **kwds)
